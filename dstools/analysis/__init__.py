...
class Corr:
    def __init__(self, df):
        ...
        self.df_corr = (
            df_corr.drop_duplicates().reset_index(drop=True).drop(columns="abs")
        )
        self.df_corr.columns = ["var1", "var2", "corr"]

    def lowest_correlation(self, var="target", threshold=0.9):
        ...
        for i in range(self.df_corr.shape[0]):
            ...
            if (line.var1 != var) & (line.var2 != var):
                ...
                cor1 = self.df_corr.loc[
                    (
                        self.df_corr["var1"].isin([line.var1, var])
                        & self.df_corr["var2"].isin([line.var1, var])
                    ),
                    "corr",
                ].values
                cor2 = self.df_corr.loc[
                    (
                        self.df_corr["var1"].isin([line.var2, var])
                        & self.df_corr["var2"].isin([line.var2, var])
                    ),
                    "corr",
                ].values

                self.df_corr.loc[i, col_var] = (
                    line.var1 if np.absolute(cor1) < np.absolute(cor2) else line.var2
                )

        return list(set(self.df_corr.loc[self.df_corr["corr"] >= 0.9, col_var]))
...
def word_counts(df, column, stop=[]):
    aux = (
        ...
        .stack()
        .reset_index(name="aux", drop=True)
    )
